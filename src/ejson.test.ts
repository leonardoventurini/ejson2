...
test('option: keyOrderSensitive', () => {
  ...
  expect(EJSON.equals({ a: {} }, { a: { b: 2 } }, { keyOrderSensitive: true })).toBeFalsy();
  expect(EJSON.equals({ a: { b: 2 } }, { a: {} }, { keyOrderSensitive: true })).toBeFalsy();
});
...
describe('NaN and Inf', () => {
  test('should handle special values in EJSON', () => {
    ...
    expect(EJSON.equals(EJSON.parse('{"a": {"$InfNaN": 1}}'), { a: Infinity })).toBe(true);
    expect(EJSON.equals(EJSON.parse('{"a": {"$InfNaN": 0}}'), { a: NaN })).toBe(true);
  });
});
...
describe('EJSON stringify functionality', () => {
  ...
  describe('handles objects', () => {
    ...
    test('with canonical setting', () => {
      expect(EJSON.stringify(testObj, { canonical: true })).toBe('{"a":1,"b":[2,{"c":3,"d":4}]}');
      expect(EJSON.stringify(testObj, { canonical: false })).toBe('{"b":[2,{"d":4,"c":3}],"a":1}');
    });
